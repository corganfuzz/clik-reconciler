...
def dict_to_hcl(obj, indent=0) -> str:
    """Format Python dictionaries as valid Terraform HCL."""
    pad = " " * indent
    if isinstance(obj, bool):
        ...
        if not obj:
            return "[]"
        items = ", ".join(dict_to_hcl(i, indent) for i in obj)
        return f"[{items}]"
...
def generate_files(instances: dict) -> None:
    locals_path = '../env/dev/imported_vms.tf'
    imports_path = '../env/dev/vm_imports.tf'

    # 1. Generate HCL locals instead of JSON
    # This allows Day 2 operations to be managed entirely inside HCL files.
